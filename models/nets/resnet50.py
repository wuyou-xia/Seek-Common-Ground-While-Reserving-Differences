...
"""
import torch
from torch import Tensor, is_distributed
...
class ResNet50(nn.Module):
    ...
    def __init__(
        ...
        self.fc2 = nn.Linear(512 * block.expansion, dim)
        ...
        if self.is_distribution:
            self.distribution_classifier = nn.Linear(dim, n_class)
    ...
    def _forward_impl(self, x):
        ...
        if self.proj:
            ...
            feat = self.fc2(feat)
            feature = self.l2norm(feat)

        out = self.fc(x)
        if self.is_remix:
            rot_output = self.rot_classifier(x)
            if self.proj:
                return out, rot_output, feature
            else:
                return out, rot_output
        else:
            if self.proj:
                if self.is_distribution:
                    distribution = self.distribution_classifier(feature)
                    return out, feature, distribution
                else:
                    return out, feature
            else:
                return out
...
class build_ResNet50:
    def __init__(self, is_remix=False, dim=64, proj=False, is_distribution=False):
        self.is_remix = is_remix
        self.dim = dim
        self.proj = proj
        self.is_distribution = is_distribution
    ...
    def build(self, num_classes):
        return ResNet50(n_class=num_classes, is_remix=self.is_remix, dim=self.dim, 
        proj=self.proj, is_distribution=self.is_distribution)
